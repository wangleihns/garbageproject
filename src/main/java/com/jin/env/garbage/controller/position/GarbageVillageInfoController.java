...
public class GarbageVillageInfoController {
    ...
    public ResponseData addGarbageVillageInfo(Integer provinceId, Long cityId, Long countryId, Long townId, Long villageId,
        ...
        Assert.state(countryId != null, "区县id不能为空");
        Assert.state(townId != null, "乡镇id不能为空");
        Assert.state(villageId != null, "村id不能为空");
        Assert.hasText(showTitle, "大数据展示屏标题不能为空");
        ResponseData responsePageData =garbageVillageInfoService.addGarbageVillageInfo(provinceId, cityId, countryId,townId, villageId,
    ...
    public ResponseData garbageVillageInfoList(Integer pageNo, Integer pageSize,
                                               Long cityId, Long countryId, Long townId, Long villageId, String search, HttpServletRequest request){
        String jwt = request.getHeader("Authorization").split(" ")[1];
        ResponseData responsePageData  = garbageVillageInfoService.garbageVillageInfoList(pageNo, pageSize, cityId,countryId, townId,villageId, search, jwt);
        return  responsePageData;
    }
}
