...
public class GarbageImageService {
    ...
    @Transactional
    public ResponseData uploadFoldImg(MultipartFile multipartFile) {
        ResponseData responseData = new ResponseData();
        String root_fileName = multipartFile.getOriginalFilename();
        String suffix  = FilenameUtils.getExtension(root_fileName);
        String uuid = UUID.randomUUID().toString();
        ...
        imageEntity.setImageSize(fileSize);
        imageEntity.setImagePath(path);
        imageEntity.setSourceName(GarbageFoldEntity.class.getName());
        imageEntity.setType(Constants.image.FOLD.name());
        imageEntity = garbageImageDao.save(imageEntity);
        responseData.setStatus(Constants.responseStatus.Success.getStatus());
